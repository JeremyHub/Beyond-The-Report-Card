...
## Map of County-level academic achievement categories in California
...
## Aggregated county-level math achievement of students of different socioeconomic status groups
...
## Aggregated county-level language arts achievement (Reasoning Through Language Arts -- RLA) of students of different socioeconomic status groups
...
```{r rla ecd performance map}
pal_rla <- colorFactor(
  palette = met.brewer(palname,6)[1:6],
  domain = summary_rla_data$mean_gcs_diff_cat
)
...
leaflet() %>%
  ...
  addPolygons(
    ...
    fillColor = ~pal_rla(mean_gcs_diff_cat),
    opacity = .25,
    color = "white",
    fillOpacity = .5,
    group = "Economically Disadvantaged",
