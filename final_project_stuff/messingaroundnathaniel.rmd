...
ggplot(data = world) +
  ...
  geom_point(data = (wide_merged_data%>%filter(!is.na(gradecenter)&!is.na(gcs_mn_avg_ol))) , aes(x=as.numeric(Longitude),y=as.numeric(Latitude),color=(as.numeric(gcs_mn_avg_ol)-as.numeric(gradecenter)),size=as.numeric(ncesenroll)), alpha =.25) +
  theme_classic() +
...
ggplot(data = world) +
  ...
  scale_color_met_c("Benedictus",direction = 1) +
  scale_size_area(max_size = 2) + theme_nr() +
  labs(x="Longitude",y="Latitude",color="Grade Cohort Score",title = "California School Location and Per Student\nSpending")+guides(size=FALSE)+theme(legend.position = c(.8,.8))
```
