...
ggplot(data = world) +
  geom_sf(data = counties, fill='#f0f1f9') + 
  geom_point(data = (wide_merged_data%>%filter(!is.na(gradecenter)&!is.na(gcs_mn_avg_ol))) , aes(x=as.numeric(Longitude),y=as.numeric(Latitude),color=(as.numeric(gcs_mn_avg_ol)-as.numeric(gradecenter)),size=as.numeric(ncesenroll)), alpha =.5, size=.5) +
  theme_classic() +
  scale_color_met_c("Benedictus",direction = 1) +
  scale_size_area(max_size = 7) + theme_nr() +
