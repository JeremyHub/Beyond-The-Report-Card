...
wide_merged_data %>% filter(!is.na(gradecenter)) %>%
ggplot(aes(x=as.numeric(Longitude),y=as.numeric(Latitude),color=(as.numeric(gcs_mn_avg_ol)-as.numeric(gradecenter)))) +
  geom_point(alpha=.5) + theme_classic()
wide_merged_data$MTH_PCTPROF_max[!is.na(wide_merged_data$MTH_PCTPROF_max) & !is.na(wide_merged_data$gradecenter)]
```
