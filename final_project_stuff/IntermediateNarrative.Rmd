...
**Educational Opportunity Project at Stanford University (SEDA)** [Codebook](https://github.com/JeremyHub/STAT-456-Final/blob/82e84dd90db010d96edba2d74dc574c15648a10c/final_project_stuff/stanford_codebook.xlsx) 
...
ggplot() + 
  ...
  scale_fill_viridis_d(labels=c('1-2 grades lower than national average', 'Less than 1 grade lower than average', '0-1 grade higher than national average')) +
...
ggplot(data = world) +
  ...
  geom_point(data = (wide_merged_data%>%filter(!is.na(gradecenter)&!is.na(gcs_mn_avg_ol))) , aes(x=as.numeric(Longitude),y=as.numeric(Latitude),color=as.numeric(pp_total_raw),size=as.numeric(ncesenroll)), alpha =.5) + 
  theme_minimal() +
  scale_color_gradient(low='#FFE400', high = '#611BB8', limits=c(8000,18000)) +
  scale_size_area(max_size = 2) +
  labs(x="Longitude",y="Latitude",color="Raw Spending",title = "California School Location and Spending")+guides(size=FALSE)+theme(legend.position = c(.8,.8))
```
