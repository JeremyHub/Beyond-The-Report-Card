...
do_lasso<-function(variable, data, labels, y_nudge, x_nudge) {
    ...
    coefs_lambdas <- 
      ...
      rename(`% Asian` = perasn, `% Black` = perblk, `% Economically Disadvantaged` = perecd, `% Free/Reducded Lunch` = perfl, `Serves Grades 6-9` = GSserved_X6.9, `Serves Grades K-12` = gradespan_KG.12) %>%
...
To answer the research question of what defines the differences between metrics we look at the predictors for each metric. That means that we look at what factors have a significant effect (either positive or negative) on different metrics of school / individual educational outcome.

We will compare some of the different metrics and scores that we have looked at previously such as ELA Test Score, GCS Score, and both ELA and Math growth scores.

We will be looking at a school level for this analysis. That means that all of our predictors are aggregated at a school level. For example the predictor of free/reduced lunch is the percentage of students at that school that have free/reduced lunch.
...
```{r perform lasso pt1}
do_lasso(outcome$ela_data_currstatus_ALL, predictors, labs(title="Significant Predictors of ELA Test Score"), c(0.1, -0.1, -0.1, -0.1), 0.03)
do_lasso(outcome$gcs, predictors, labs(title="Significant Predictors of GCS Score"), c(0.1, -0.1, -0.1), 0.03)
```

## ELA & Math Growth Scores

Next we look at the growth scores for ELA and Math. The first thing to notice about these graphs is that all the predictors converge to zero much faster than the previous two graphs. We can tell this by looking at the absolute numbers on the X axis. On the first two graphs it took until around 0.1 - 0.15 penalty for all predictors to reach zero. In these graphs it takes a much smaller penalty (roughly 0.01 - 0.03) for all predictors to reach zero significance. This tells us that none of the predictors are as significant as the previous graphs. However, there are some marginally significant predictors. Namely the ones seen previously as well as two new categories for grades served by the school.

```{r perform lasso pt2}
do_lasso(outcome$growthscore_ela, predictors, labs(title="Significant Predictors of ELA Test Score Growth"), -0.1, 0.005)
do_lasso(outcome$growthscore_math, predictors, labs(title="Significant Predictors of Math Test Score Growth"), c(0.15, -0.1, -0.1), 0.01)
```
