...
library(tidyverse)
```
In this tutorial, we will use `neuralnet` to perform Neural Networks in R. We intended to use `keras` and `tensorflow` for this tutorial as well, but unfortunately it does not run on M1 laptops. You can read more in the Resources section in the Google Docs tutorial.
...
+ `model` is what variables the `neuralnet` should take into account and what it is trying to predict. If you have done any modeling in 155 or 253, this will look very familiar. Feel free to change the model and see what happens.

+ `data` is the data to use (we will use the data we reserved for training)

+ `hidden` is the number of hidden nodes that the network will have. Increasing these numbers and / or adding more layers (numbers) to the list will increase training time but can potentially improve model performance.

+ `linear.output` tells the model if what it is trying to predict is categorical or numerical in nature. We are trying to predict house price, which is numerical, so we will set it to TRUE.
...
We can use the function `plot(model)` to see the shape and the parameters belonging to each of the layer in our neural network model.
